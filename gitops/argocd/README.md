...
6. Disparar uma preview: abrir uma PR real no GitHub (no seu fork) com a label `preview` alterando `apps/apps/go-web/src/*`. O workflow builda e publica `ghcr.io/<seu-usuario>/go-web:pr-<numero>-<head-short-sha>` e, em até 15s, o `ApplicationSet` cria a `Application`/namespace `pre-env-<branch-slug>-<numero>`.
